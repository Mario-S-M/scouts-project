...
export class AvisosSalidaService {
  ...
  constructor(
    ...
    if (fs.existsSync(grupoPath))
      this.logoGrupo = `data:image/png;base64,${fs.readFileSync(grupoPath).toString("base64")}`;
    if (fs.existsSync(ccPath))
      this.logoCC = `data:image/png;base64,${fs.readFileSync(ccPath).toString("base64")}`;
    ...
    const fontBuffers: Record<string, Buffer> = {
      "Roboto-Regular.ttf": Buffer.from(
      ...
      "Roboto-MediumItalic.ttf": Buffer.from(
  ...
  private async fetchMapImage(googleUrl: string): Promise<string | null> {
    try {
      ...
      const W = 1500,
        ...
        TILE = 256;
      const W2 = W,
        H2 = H; // sin segundo escale, directo al tamaño final
      ...
      const left = Math.min(
        Math.max(0, centerPxX - Math.floor(W2 / 2)),
        gridW - W2,
      );
      const top = Math.min(
        Math.max(0, centerPxY - Math.floor(H2 / 2)),
        gridH - H2,
      );
  ...
  private buildDocument(dto: AvisoSalidaDto, mapImage?: string | null): any {
    const scoutersValidos = (dto.scouters ?? []).filter((s) =>
      s.nombre?.trim(),
    );
    const invitadosValidos = (dto.invitados ?? []).filter((i) =>
      i.nombre?.trim(),
    );
    const participantes = dto.participantes ?? [];
    const totalAsistentes =
      participantes.length + scoutersValidos.length + invitadosValidos.length;
    ...
    return {
      ...
      content: [
        ...
        ...["manada", "tropa", "comunidad", "clan"]
          .map((sec) =>
            this.seccionTable(
              participantes.filter((p) => p.seccion === sec),
              sec,
            ),
          )
          .filter(Boolean),
        invitadosValidos.length
          ? this.invitadosSection(invitadosValidos)
          : null,
        this.firmas(),
      ].filter(Boolean),
    };
        ...
        body: [
          [
            leftCell,
            {
              stack: [
                {
                  text: "ASOCIACIÓN DE SCOUTS DE MÉXICO, A.C.",
                  style: "headerTitle",
                  alignment: "center",
                },
                {
                  text: "PROVINCIA MICHOACÁN  ·  GRUPO 7 MORELIA",
                  style: "headerSub",
                  alignment: "center",
                  margin: [0, 2, 0, 0],
                },
        ...
        },
        {
          ...
          table: {
            ...
            body: [
              [
                {
                  text: C.secciones[sec].label,
                  fillColor: C.secciones[sec].bg,
                  color: C.secciones[sec].text,
                  bold: true,
                  fontSize: 8,
                  alignment: "center",
                  margin: [2, 2, 2, 2],
  ...
  private contactGrid(dto: AvisoSalidaDto): any {
    const contactBlock = (label: string, c: any) => ({
      stack: [
        this.pill(label),
        {
          text: [{ text: "Nombre: ", bold: true }, c?.nombre || "—"],
          margin: [0, 2, 0, 1],
        },
        {
          text: [{ text: "Cargo: ", bold: true }, c?.cargo || "—"],
          margin: [0, 1, 0, 1],
        },
        {
          text: [{ text: "Teléfono: ", bold: true }, c?.telefono || "—"],
          margin: [0, 1, 0, 2],
        },
      ],
      fillColor: C.white,
      ...
      stack: [
        ...
        this.box(
          [
            {
              text:
                dto.lugarDescripcion ||
                "Sin descripción adicional del lugar de la actividad.",
              margin: [0, 2, 0, mapImage ? 6 : 2],
            },
            mapImage
              ? { image: "mapImg", width: 490, alignment: "center" }
              : null,
          ].filter(Boolean),
        ),
      ],
          ...
          table: {
            ...
            body: [
              ...
              ...participantes.map((p, i) => [
                ...
                {
                  text: fmtDate(p.fechaNacimiento),
                  alignment: "center",
                  fontSize: 8,
                },
              ]),
  ...
  private buildPermisosDocument(dto: AvisoSalidaDto): any {
    ...
    const targets: (ParticipanteDto | null)[] =
      participantes.length > 0 ? participantes : [null];
  ...
  private buildPermisoPage(
    ...
    const contactos = [dto.contactoLocal, dto.contactoActividad].filter((c) =>
      c?.nombre?.trim(),
    );
    ...
    const cbox = () => ({
      table: {
        widths: [11],
        body: [[{ text: " ", fillColor: INPUT, margin: [0, 2, 0, 2] }]],
      },
      layout: {
        hLineColor: () => BORDER,
        vLineColor: () => BORDER,
    ...
    return [
            ...
            [
              {
                stack: [
                  ...
                  {
                    text: [
                      { text: "• Punto de Reunión: ", bold: true },
                      dto.salida?.lugar || "—",
                    ],
                    fontSize: 8,
                    margin: [0, 2, 0, 1],
              ...
              },
              {
                stack: [
                  { text: "Salida:", bold: true, fontSize: 9 },
                  {
                    text: [
                      { text: "• Punto de Reunión: ", bold: true },
                      dto.llegada?.lugar || dto.salida?.lugar || "—",
                    ],
                    fontSize: 8,
                    margin: [0, 2, 0, 1],
                  },
      ...
      // ─ AUTORIZO ─
      {
        columns: [
          {
            width: "auto",
            text: [{ text: "AUTORIZO", bold: true }, " a mi hijo (a):"],
            fontSize: 9,
          },
      ...
      // ─ Sangre + Teléfono ─
      {
        columns: [
          {
            width: "auto",
            text: ["Su tipo de ", { text: "sangre", bold: true }, " es:"],
            fontSize: 8.5,
          },
          { ...ibox(), width: 50, margin: [4, 0, 10, 0] },
          {
            width: "auto",
            text: [{ text: "Teléfono", bold: true }, " de emergencia:"],
            fontSize: 8.5,
          },
        ...
        columns: [
          {
            width: "auto",
            text: [
              { text: "Problemas graves de Salud", bold: true },
              " que le impidan realizar alguna actividad:",
            ],
            fontSize: 8.5,
          },
          { ...ibox(), width: "*", margin: [4, 0, 0, 0] },
        ...
        columns: [
          {
            width: "auto",
            text: [
              "Solicito acudan al hospital, clínica o ",
              { text: "centro de Salud", bold: true },
              ":",
            ],
            fontSize: 8.5,
          },
      ...
      // ─ Seguro Popular ─
      {
        columns: [
          {
            width: "auto",
            text: ["No. POLIZA SEGURO ", { text: "POPULAR", bold: true }, ":"],
            fontSize: 8.5,
          },
          { ...ibox(), width: 160, margin: [4, 0, 0, 0] },
        ],
      ...
      // ─ Nombre del Joven ─
      {
        columns: [
          {
            width: "auto",
            text: [{ text: "Nombre del Joven:", bold: true }],
            fontSize: 9,
          },
          { ...ibox(nombreJoven), width: "*", margin: [4, 0, 0, 0] },
        ],
  ...
  async update(id: number, dto: AvisoSalidaDto): Promise<AvisoSalida> {
    ...
    aviso.fechaSalida = dto.salida?.fecha
      ? dto.salida.fecha.slice(0, 10)
      : aviso.fechaSalida;
    aviso.data = dto;
